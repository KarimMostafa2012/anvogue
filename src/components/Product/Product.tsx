...
const Product: React.FC<ProductProps> = ({ data, type }) => {
  ...
  useEffect(() => {
    ...
    (currentLanguage == "ar" || currentLanguage == "ckb") ? setDirection("right") : setDirection("left");
  }, [currentLanguage]);
  ...
  const handleAddToWishlist = () => {
    ...
    if (wishlistState.wishlistArray.some((item) => item.id === Number(data.id))) {
            ...
            <div className="product-thumb bg-white relative overflow-hidden rounded-2xl">
              ...
              {style === "style-1" ||
                ...
                <div className="list-action-right absolute top-3 right-3 max-lg:hidden">
                  {style === "style-4" && (
                    <div
                      className={`add-cart-btn w-[32px] h-[32px] flex items-center justify-center rounded-full bg-white duration-300 relative mb-2 ${compareState.compareArray.some(
              ...
              {data.has_offer && (
                <>
                  <Marquee direction={direction} className="banner-sale-auto bg-black absolute bottom-0 left-0 w-full py-1.5">
                    <div
                    ...
                    <>
                      ...
                      >
                        <div className="list-size flex items-center justify-center flex-wrap gap-2">
                          <div
                            className={`size-item w-10 h-10 rounded-full flex items-center justify-center text-button bg-white border border-line ${data.size ? "active" : ""
                              }`}
                          >
                            {data.size}
                          </div>
                      ...
                      {data.colors?.length > 0 && (
                        <div className="list-color max-md:hidden py-2 mt-5 mb-1 flex items-center gap-3 flex-wrap duration-300 !static">
                          {data.colors?.map((item, index) => (
                            <div
                              key={index}
                              className={`color-item w-8 h-8 rounded-full duration-300 relative !border-[1px] !border-[rgba(0,0,0,0.4)] ${activeColor === item.color ? "active" : ""
                                }`}
                              style={{ backgroundColor: `${item.color}` }}
                              onClick={(e) => {
                                e.stopPropagation();
